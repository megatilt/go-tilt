...
func (b *BlockGen) OffsetTime(seconds int64) {
	...
	b.header.Difficulty = tilthash.CalcDifficulty(b.config, b.header.Time.Uint64(), b.parent.Time().Uint64(), b.parent.Number(), b.parent.Difficulty())
}
...
func GenerateChain(config *params.ChainConfig, parent *types.Block, db tiltdb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config == nil {
	...
	genblock := func(i int, h *types.Header, statedb *state.StateDB) (*types.Block, types.Receipts) {
		...
		tilthash.AccumulateRewards(statedb, h, b.uncles)
...
func makeHeader(config *params.ChainConfig, parent *types.Block, state *state.StateDB) *types.Header {
	...
	return &types.Header{
		...
		Difficulty: tilthash.CalcDifficulty(config, time.Uint64(), new(big.Int).Sub(time, big.NewInt(10)).Uint64(), parent.Number(), parent.Difficulty()),
...
// header only chain.
func newCanonical(n int, full bool) (tiltdb.Database, *BlockChain, error) {
	// Initialize a fresh chain with only a genesis block
	gspec := new(Genesis)
	db, _ := tiltdb.NewMemDatabase()
	genesis := gspec.MustCommit(db)

	blockchain, _ := NewBlockChain(db, params.AllProtocolChanges, tilthash.NewFaker(), new(event.TypeMux), vm.Config{})
...
func makeHeaderChain(parent *types.Header, n int, db tiltdb.Database, seed int) []*types.Header {
...
func makeBlockChain(parent *types.Block, n int, db tiltdb.Database, seed int) []*types.Block {
