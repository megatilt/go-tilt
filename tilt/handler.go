...
import (
	...
	"github.com/megatilt/go-tilt/tilt/fetcher"
...
// NewProtocolManager returns a new tiltnet sub protocol manager. The Tiltnet sub protocol manages peers capable
// with the tiltnet network.
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, maxPeers int, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb tiltdb.Database) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Tiltnet peer", "peer", id)

	// Unregister the peer from the downloader and Tiltnet peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Tiltnet protocol")
	...
	log.Info("Tiltnet protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an tilt peer. When
// this function terminates, the peer is disconnected.
func (pm *ProtocolManager) handle(p *peer) error {
	if pm.peers.Len() >= pm.maxPeers {
		return p2p.DiscTooManyPeers
	}
	p.Log().Debug("Tiltnet peer connected", "name", p.Name())

	// Execute the Tiltnet handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("Tiltnet handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Tiltnet peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Tiltnet message handling failed", "err", err)
			return err
		}
	}
}
...
// TiltNodeInfo represents a short summary of the Tiltnet sub-protocol metadata known
// about the host peer.
type TiltNodeInfo struct {
	Network    uint64      `json:"network"`    // Tiltnet network ID
...
func (self *ProtocolManager) NodeInfo() *TiltNodeInfo {
