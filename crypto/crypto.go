...
// Creates an tiltnet address given the bytes and the nonce
...
func ValidateSignatureValues(v byte, r, s *big.Int, omaha bool) bool {
	if r.Cmp(common.Big1) < 0 || s.Cmp(common.Big1) < 0 {
		return false
	}
	// reject upper range of s values (ECDSA malleability)
	// see discussion in secp256k1/libsecp256k1/include/secp256k1.h
	if omaha && s.Cmp(secp256k1_halfN) > 0 {
		return false
	}
	// Holdem: allow s to be in full N range
