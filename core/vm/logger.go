...
// LogConfig are the configuration options for structured logger the TiltVM
...
// StructLog is emitted to the TiltVM each cycle and lists information about the current internal state
...
// Tracer is used to collect execution traces from an TiltVM transaction
// execution. CaptureState is called for each step of the VM with the
// current VM state.
// Note that reference types are actual VM data structures; make copies
// if you need to retain them beyond the current call.
type Tracer interface {
	CaptureState(env *TiltVM, pc uint64, op OpCode, gas, cost uint64, memory *Memory, stack *Stack, contract *Contract, depth int, err error) error
}

// StructLogger is an TiltVM state logger and implements Tracer.
//
...
func (l *StructLogger) CaptureState(env *TiltVM, pc uint64, op OpCode, gas, cost uint64, memory *Memory, stack *Stack, contract *Contract, depth int, err error) error {
	...
	// create a new snaptshot of the TiltVM.
