...
import "github.com/megatilt/go-tilt/accounts/abi"
...
type tmplContract struct {
	...
	InputBin    string                 // Optional TiltVM bytecode used to denetare deploy code from
...
{{range $contract := .Contracts}}
	...
	{{if .InputBin}}
		...
		// Deploy{{.Type}} deploys a new Tiltnet contract, binding an instance of {{.Type}} to it.
	...
	// {{.Type}} is an auto generated Go binding around an Tiltnet contract.
	type {{.Type}} struct {
	  {{.Type}}Caller     // Read-only binding to the contract
	  {{.Type}}Transactor // Write-only binding to the contract
	}

	// {{.Type}}Caller is an auto generated read-only Go binding around an Tiltnet contract.
	type {{.Type}}Caller struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Transactor is an auto generated write-only Go binding around an Tiltnet contract.
	type {{.Type}}Transactor struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Session is an auto generated Go binding around an Tiltnet contract,
	...
	// {{.Type}}CallerSession is an auto generated read-only Go binding around an Tiltnet contract,
	...
	// {{.Type}}TransactorSession is an auto generated write-only Go binding around an Tiltnet contract,
	...
	// {{.Type}}Raw is an auto generated low-level Go binding around an Tiltnet contract.
	type {{.Type}}Raw struct {
	  Contract *{{.Type}} // Generic contract binding to access the raw methods on
	}

	// {{.Type}}CallerRaw is an auto generated low-level read-only Go binding around an Tiltnet contract.
	type {{.Type}}CallerRaw struct {
		Contract *{{.Type}}Caller // Generic read-only contract binding to access the raw methods on
	}

	// {{.Type}}TransactorRaw is an auto generated low-level write-only Go binding around an Tiltnet contract.
...
import org.tiltnet.tiltnode.*;
import org.tiltnet.tiltnode.internal.*;
...
{{range $contract := .Contracts}}
	public class {{.Type}} {
		...
		{{if .InputBin}}
			...
			// deploy deploys a new Tiltnet contract, binding an instance of {{.Type}} to it.
			public static {{.Type}} deploy(TransactOpts auth, TiltnetClient client{{range .Constructor.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
				Interfaces args = Tiltnode.newInterfaces({{(len .Constructor.Inputs)}});
				{{range $index, $element := .Constructor.Inputs}}
				  args.set({{$index}}, Tiltnode.newInterface()); args.get({{$index}}).set{{namedtype (bindtype .Type) .Type}}({{.Name}});
				{{end}}
				return new {{.Type}}(Tiltnode.deployContract(auth, ABI, BYTECODE, client, args));
			}
		...
		// Tiltnet address where this contract is located at.
		public final Address Address;

		// Tiltnet transaction in which this contract was deployed (if known!).
		...
		public {{.Type}}(Address address, TiltnetClient client) throws Exception {
			this(Tiltnode.bindContract(address, ABI, client));
		}
		...
		{{range .Calls}}
			...
			public {{if gt (len .Normalized.Outputs) 1}}{{capitalise .Normalized.Name}}Results{{else}}{{range .Normalized.Outputs}}{{bindtype .Type}}{{end}}{{end}} {{.Normalized.Name}}(CallOpts opts{{range .Normalized.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
				Interfaces args = Tiltnode.newInterfaces({{(len .Normalized.Inputs)}});
				{{range $index, $item := .Normalized.Inputs}}args.set({{$index}}, Tiltnode.newInterface()); args.get({{$index}}).set{{namedtype (bindtype .Type) .Type}}({{.Name}});
				{{end}}

				Interfaces results = Tiltnode.newInterfaces({{(len .Normalized.Outputs)}});
				{{range $index, $item := .Normalized.Outputs}}Interface result{{$index}} = Tiltnode.newInterface(); result{{$index}}.setDefault{{namedtype (bindtype .Type) .Type}}(); results.set({{$index}}, result{{$index}});
				{{end}}

				if (opts == null) {
					opts = Tiltnode.newCallOpts();
				}
