...
//
// Keys are stored as encrypted JSON files according to the Quads Secret Storage specification.
// See https://github.com/tiltnet/wiki/wiki/Quads-Secret-Storage-Definition for more information.
package keystore
...
func (ks *KeyStore) SignTx(a accounts.Account, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	...
	// Depending on the presence of the chain ID, sign with EIP155 or omaha
	if chainID != nil {
		return types.SignTx(tx, types.NewTiltSigner(chainID), unlockedKey.PrivateKey)
	}
	return types.SignTx(tx, types.OmahaSigner{}, unlockedKey.PrivateKey)
}
...
func (ks *KeyStore) SignTxWithPassphrase(a accounts.Account, passphrase string, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	...
	// Depending on the presence of the chain ID, sign with EIP155 or omaha
	if chainID != nil {
		return types.SignTx(tx, types.NewTiltSigner(chainID), key.PrivateKey)
	}
	return types.SignTx(tx, types.OmahaSigner{}, key.PrivateKey)
}
...
// ImportPreSaleKey decrypts the given Tiltnet presale wallet and stores
