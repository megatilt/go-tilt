...
import (
	...
	"github.com/megatilt/go-tilt/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of tiltnode is "tiltnode". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Tiltnode", keep that.
	if name == "tiltnode" || name == "tiltnode-testnet" {
		name = "Tiltnode"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "tiltnode" instances.
var isOldTiltnodeResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by tiltnode 1.4 are used if they exist.
	if c.name() == "tiltnode" && isOldTiltnodeResource[path] {
		oldpath := ""
		if c.Name == "tiltnode" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	default:
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-tiltnet-keystore")
		ephemeral = keydir
	}
	if err != nil {
		return nil, "", err
