...
func MakeSigner(config *params.ChainConfig, blockNumber *big.Int) Signer {
	signer := NewTiltSigner(config.ChainId)
	return signer
}
...
// EIP155 rules
type TiltSigner struct {
	OmahaSigner

	chainId, chainIdMul *big.Int
}

func NewTiltSigner(chainId *big.Int) TiltSigner {
	if chainId == nil {
		chainId = new(big.Int)
	}
	return TiltSigner{
...
func (s TiltSigner) Equal(s2 Signer) bool {
	eip155, ok := s2.(TiltSigner)
	return ok && eip155.chainId.Cmp(s.chainId) == 0
}

func (s TiltSigner) PublicKey(tx *Transaction) ([]byte, error) {
	// if the transaction is not protected fall back to omaha signer
	if !tx.Protected() {
		return (OmahaSigner{}).PublicKey(tx)
	}
...
func (s TiltSigner) WithSignature(tx *Transaction, sig []byte) (*Transaction, error) {
	if len(sig) != 65 {
...
func (s TiltSigner) Hash(tx *Transaction) common.Hash {
...
// OmahaTransaction implements TransactionInterface using the
// omaha rules.
type OmahaSigner struct{ HoldemSigner }
...
// WithSignature returns a new transaction with the given signature. This signature
// needs to be in the [R || S || V] format where V is 0 or 1.
func (hs OmahaSigner) WithSignature(tx *Transaction, sig []byte) (*Transaction, error) {
	if len(sig) != 65 {
...
func (hs OmahaSigner) PublicKey(tx *Transaction) ([]byte, error) {
...
type HoldemSigner struct{}

func (s HoldemSigner) Equal(s2 Signer) bool {
	_, ok := s2.(HoldemSigner)
	return ok
}

// WithSignature returns a new transaction with the given signature. This signature
// needs to be in the [R || S || V] format where V is 0 or 1.
func (fs HoldemSigner) WithSignature(tx *Transaction, sig []byte) (*Transaction, error) {
	if len(sig) != 65 {
		panic(fmt.Sprintf("wrong size for snature: got %d, want 65", len(sig)))
	}
	cpy := &Transaction{data: tx.data}
...
// It does not uniquely identify the transaction.
func (fs HoldemSigner) Hash(tx *Transaction) common.Hash {
	return rlpHash([]interface{}{
		tx.data.AccountNonce,
		tx.data.Price,
		tx.data.GasLimit,
...
func (fs HoldemSigner) PublicKey(tx *Transaction) ([]byte, error) {
	if tx.data.V.BitLen() > 8 {
		return nil, ErrInvalidSig
	}
