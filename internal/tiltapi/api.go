...
const (
	defaultGas      = 90000
	defaultGasPrice = 50 * params.Blom
	emptyHex        = "0x"
)

// PublicTiltnetAPI provides an API to access Tiltnet related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicTiltnetAPI struct {
	b Backend
}

// NewPublicTiltnetAPI creates a new Tiltnet protocol API.
func NewPublicTiltnetAPI(b Backend) *PublicTiltnetAPI {
	return &PublicTiltnetAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicTiltnetAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Tiltnet protocol version this node supports
func (s *PublicTiltnetAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicTiltnetAPI) Syncing() (interface{}, error) {
...
func (s *PublicTxPoolAPI) Inspect() map[string]map[string]map[string]string {
	...
	var format = func(tx *types.Transaction) string {
		if to := tx.To(); to != nil {
			return fmt.Sprintf("%s: %v dwan + %v × %v gas", tx.To().Hex(), tx.Value(), tx.Gas(), tx.GasPrice())
		}
		return fmt.Sprintf("contract creation: %v dwan + %v × %v gas", tx.Value(), tx.Gas(), tx.GasPrice())
	}
	// Flatten the pending transactions
	for account, txs := range pending {
...
//   keccak256("\x19Tiltnet Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Tiltnet Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Tiltnet ECDSA signature for:
// keccack256("\x19Tiltnet Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/megatilt/go-tilt/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with tilt_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Tiltnet Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/megatilt/go-tilt/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Tiltnet signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Tiltnet blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Tiltnet blockchain API.
...
// GetBalance returns the amount of dwan for the given address in the state of the
...
func (s *PublicBlockChainAPI) doCall(ctx context.Context, args CallArgs, blockNr rpc.BlockNumber, vmCfg vm.Config) ([]byte, *big.Int, error) {
	defer func(start time.Time) { log.Debug("Executing TiltVM call finished", "runtime", time.Since(start)) }(time.Now())
	...
	// Get a new instance of the TiltVM.
	tiltvm, vmError, err := s.b.GetTiltVM(ctx, msg, state, header, vmCfg)
	if err != nil {
		return nil, common.Big0, err
	}
	// Wait for the context to be done and cancel the tiltvm. Even if the
	// TiltVM has finished, cancelling may be done (repeatedly)
	go func() {
		select {
		case <-ctx.Done():
			tiltvm.Cancel()
	...
	res, gas, err := core.ApplyMessage(tiltvm, msg, gp)
...
// ExecutionResult groups all structured logs emitted by the TiltVM
...
// StructLogRes stores a structured log emitted by the TiltVM while replaying a
...
// formatLogs formats TiltVM returned structured logs for json output
...
func newRPCPendingTransaction(tx *types.Transaction) *RPCTransaction {
	var signer types.Signer = types.HoldemSigner{}
	if tx.Protected() {
		signer = types.NewTiltSigner(tx.ChainId())
	}
...
func getTransaction(chainDb tiltdb.Database, b Backend, txHash common.Hash) (*types.Transaction, bool, error) {
...
func getTransactionBlockData(chainDb tiltdb.Database, txHash common.Hash) (common.Hash, uint64, uint64, error) {
	var txBlock struct {
...
//
// https://github.com/tiltnet/wiki/wiki/JSON-RPC#tilt_sign
...
func (s *PublicTransactionPoolAPI) PendingTransactions() ([]*RPCTransaction, error) {
	...
	for _, tx := range pending {
		var signer types.Signer = types.OmahaSigner{}
		if tx.Protected() {
			signer = types.NewTiltSigner(tx.ChainId())
		}
		from, _ := types.Sender(signer, tx)
...
func (s *PublicTransactionPoolAPI) Resend(ctx context.Context, sendArgs SendTxArgs, gasPrice, gasLimit *hexutil.Big) (common.Hash, error) {
	...
	for _, p := range pending {
		var signer types.Signer = types.OmahaSigner{}
		if p.Protected() {
			signer = types.NewTiltSigner(p.ChainId())
		}
...
// PublicDebugAPI is the collection of Tiltnet APIs exposed over the public
// debugging endpoint.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", tilthash.SeedHash(number)), nil
}

// PrivateDebugAPI is the collection of Tiltnet APIs exposed over the private
// debugging endpoint.
...
// Version returns the current tiltnet protocol version.
