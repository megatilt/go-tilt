...
import (
	...
	"github.com/megatilt/go-tilt/accounts/abi/bind"
...
type SimulatedBackend struct {
	database   tiltdb.Database   // In memory database to store our testing data
	blockchain *core.BlockChain // Tiltnet blockchain to handle the consensus
...
func NewSimulatedBackend(alloc core.GenesisAlloc) *SimulatedBackend {
	database, _ := tiltdb.NewMemDatabase()
	genesis := core.Genesis{Config: params.AllProtocolChanges, Alloc: alloc}
	genesis.MustCommit(database)
	blockchain, _ := core.NewBlockChain(database, genesis.Config, tilthash.NewFaker(), new(event.TypeMux), vm.Config{})
...
// BalanceAt returns the dwan balance of a certain account in the blockchain.
...
func (b *SimulatedBackend) CallContract(ctx context.Context, call tiltnet.CallMsg, blockNumber *big.Int) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) PendingCallContract(ctx context.Context, call tiltnet.CallMsg) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) EstimateGas(ctx context.Context, call tiltnet.CallMsg) (*big.Int, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) callContract(ctx context.Context, call tiltnet.CallMsg, block *types.Block, statedb *state.StateDB) ([]byte, *big.Int, error) {
	...
	tiltvmContext := core.NewTiltVMContext(msg, block.Header(), b.blockchain, nil)
	// Create a new environment which holds all relevant information
	// about the transaction and calling mechanisms.
	vmenv := vm.NewTiltVM(tiltvmContext, statedb, b.config, vm.Config{})
...
func (b *SimulatedBackend) SendTransaction(ctx context.Context, tx *types.Transaction) error {
	...
	sender, err := types.Sender(types.OmahaSigner{}, tx)
	if err != nil {
