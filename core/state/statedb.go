...
// Package state provides a caching layer atop the Tiltnet state trie.
package state
...
// StateDBs within the tiltnet protocol are used to store anything
...
func New(root common.Hash, db tiltdb.Database) (*StateDB, error) {
...
//
// CreateAccount is called during the TiltVM CREATE operation. The situation might arise that
...
func (s *StateDB) CommitBatch() (root common.Hash, batch tiltdb.Batch) {
	batch = s.db.NewBatch()
	root, _ = s.CommitTo(batch)
...
func (s *StateDB) CommitTo(dbw trie.DatabaseWriter) (root common.Hash, err error) {
