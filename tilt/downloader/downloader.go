...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [tilt/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	newPeerCh     chan *peer
	headerCh      chan dataPack        // [tilt/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [tilt/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [tilt/63] Channel receiving inbound receipts
	stateCh       chan dataPack        // [tilt/63] Channel receiving inbound node state data
	bodyWakeCh    chan bool            // [tilt/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [tilt/63] Channel to signal the receipt fetcher of new tasks
	stateWakeCh   chan bool            // [tilt/63] Channel to signal the state fetcher of new tasks
	headerProcCh  chan []*types.Header // [tilt/62] Channel to feed the header processor new tasks
...
func New(mode SyncMode, stateDb tiltdb.Database, mux *event.TypeMux, hasHeader headerCheckFn, hasBlockAndState blockAndStateCheckFn,
...
// these are zero.
func (d *Downloader) Progress() tiltnet.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peer, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "tilt", p.version, "head", hash, "td", td, "mode", d.mode)
