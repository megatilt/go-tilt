...
// Process processes the state changes according to the Tiltnet rules by running
...
func ApplyTransaction(config *params.ChainConfig, bc *BlockChain, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *big.Int, cfg vm.Config) (*types.Receipt, *big.Int, error) {
	...
	// Create a new context to be used in the TiltVM environment
	context := NewTiltVMContext(msg, header, bc, author)
	// Create a new environment which holds all relevant information
	// about the transaction and calling mechanisms.
	vmenv := vm.NewTiltVM(context, statedb, config, cfg)
