...
// Package bind generates Tiltnet contract Go bindings.
//
// Detailed usage document and tutorial available on the go-tiltnet Wiki page:
// https://github.com/megatilt/go-tilt/wiki/Native-DApps:-Go-bindings-to-Tiltnet-contracts
package bind
...
func Bind(types []string, abis []string, bytecodes []string, pkg string, lang Lang) (string, error) {
	...
	for i := 0; i < len(types); i++ {
		...
		tiltvmABI, err := abi.JSON(strings.NewReader(abis[i]))
		if err != nil {
			return "", err
		}
		...
		for _, original := range tiltvmABI.Methods {
		...
		contracts[types[i]] = &tmplContract{
			...
			Constructor: tiltvmABI.Constructor,
			Calls:       calls,
			Transacts:   transacts,
		}
	}
