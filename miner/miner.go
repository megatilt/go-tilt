...
// Package miner implements Tiltnet block creation and mining.
package miner
...
func New(tilt Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		tilt:     tilt,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, tilt, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(tilt.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setTiltbase(coinbase)
...
func (self *Miner) SetTiltbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setTiltbase(addr)
}
