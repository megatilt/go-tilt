...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, tilt Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:         config,
		engine:         engine,
		tilt:           tilt,
		mux:            mux,
		chainDb:        tilt.ChainDb(),
		recv:           make(chan *Result, resultQueueSize),
		gasPrice:       new(big.Int),
		chain:          tilt.BlockChain(),
		proc:           tilt.BlockChain().Validator(),
		possibleUncles: make(map[common.Hash]*types.Block),
		coinbase:       coinbase,
		txQueue:        make(map[common.Hash]*types.Transaction),
		agents:         make(map[Agent]struct{}),
		unconfirmed:    newUnconfirmedBlocks(tilt.BlockChain(), 5),
...
func (self *worker) setTiltbase(addr common.Address) {
	self.mu.Lock()
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	work := &Work{
		config:    self.config,
		signer:    types.NewTiltSigner(self.config.ChainId),
		state:     state,
		ancestors: set.New(),
		family:    set.New(),
		uncles:    set.New(),
	...
	wallets := self.tilt.AccountManager().Wallets()
...
func (self *worker) commitNewWork() {
	...
	work := self.current
	pending, err := self.tilt.TxPool().Pending()
	if err != nil {
	...
	self.tilt.TxPool().RemoveBatch(work.lowGasTxs)
	self.tilt.TxPool().RemoveBatch(work.failedTxs)
