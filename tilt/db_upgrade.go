...
// been safely stopped.
func upgradeSequentialKeys(db tiltdb.Database) (stopFn func()) {
...
func upgradeSequentialCanonicalNumbers(db tiltdb.Database, stopFn func() bool) (error, bool) {
	prefix := []byte("block-num-")
	it := db.(*tiltdb.LDBDatabase).NewIterator()
	defer func() {
		it.Release()
	}()
	it.Seek(prefix)
...
// if successful.
func upgradeSequentialBlocks(db tiltdb.Database, stopFn func() bool) (error, bool) {
	prefix := []byte("block-")
	it := db.(*tiltdb.LDBDatabase).NewIterator()
	defer func() {
		it.Release()
...
func upgradeSequentialOrphanedReceipts(db tiltdb.Database, stopFn func() bool) (error, bool) {
...
func upgradeSequentialBlockData(db tiltdb.Database, hash []byte) error {
...
func addMipmapBloomBins(db tiltdb.Database) (err error) {
