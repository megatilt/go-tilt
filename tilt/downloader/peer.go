...
// Block header and body fetchers belonging to tilt/62 and above
...
type peer struct {
	...
	getRelHeaders  relativeHeaderFetcherFn // [tilt/62] Method to retrieve a batch of headers from an origin hash
	getAbsHeaders  absoluteHeaderFetcherFn // [tilt/62] Method to retrieve a batch of headers from an absolute position
	getBlockBodies blockBodyFetcherFn      // [tilt/62] Method to retrieve a batch of block bodies

	getReceipts receiptFetcherFn // [tilt/63] Method to retrieve a batch of block transaction receipts
	getNodeData stateFetcherFn   // [tilt/63] Method to retrieve a batch of state trie data

	version int        // Tilt protocol version number to switch strategies
...
func (p *peer) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [tilt/62+] requested on tilt/%d", p.version))
	}
...
func (p *peer) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [tilt/62+] requested on tilt/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peer) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [tilt/63+] requested on tilt/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peer) FetchNodeData(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [tilt/63+] requested on tilt/%d", p.version))
	}
	// Short circuit if the peer is already fetching
