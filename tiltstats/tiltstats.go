...
// Package tiltstats implements the network stats reporting service.
package tiltstats
...
// Service implements an Tiltnet netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	stack *node.Node // Temporary workaround, remove when API finalized

	server *p2p.Server      // Peer-to-peer server to retrieve networking infos
	tilt   *tilt.Tiltnet    // Full Tiltnet service if monitoring a full node
...
func New(url string, tiltServ *tilt.Tiltnet) (*Service, error) {
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // Tiltstats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["tilt"]; info != nil {
		network = fmt.Sprintf("%d", info.(*tilt.TiltNodeInfo).Network)
		protocol = fmt.Sprintf("tilt/%d", tilt.ProtocolVersions[0])
	}
	auth := &authMsg{
		Id: s.node,
		Info: nodeInfo{
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the tiltstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to tiltstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to tiltstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.tilt != nil {
		...
		if block == nil {
			block = s.tilt.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.tilt.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		if s.tilt != nil {
			head = s.tilt.BlockChain().CurrentHeader().Number.Int64()
		}
	...
	for i, number := range indexes {
		...
		if s.tilt != nil {
			block = s.tilt.BlockChain().GetBlockByNumber(number)
		}
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to tiltstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	var pending int
	if s.tilt != nil {
		pending, _ = s.tilt.TxPool().Stats()
	}
	// Assemble the transaction stats and send it to the server
	log.Trace("Sending pending transactions to tiltstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.tilt != nil {
		mining = s.tilt.Miner().Mining()
		hashrate = int(s.tilt.Miner().HashRate())

		sync := s.tilt.Downloader().Progress()
		syncing = s.tilt.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		gasprice = int(s.tilt.Miner().GasPrice().Uint64())
	}
	// Assemble the node stats and send it to the server
	log.Trace("Sending node details to tiltstats")
